...
const md5 = (filename: string) => crypto.createHash("md5").update(filename).digest("hex");
const _getFullPath = (relativePath: string) => path.resolve(nconf.get("upload_path"), relativePath);
...
module.exports = function (User: any) {
  User.associateUpload = async (uid: number, relativePath: string) => {
  ...
  User.deleteUpload = async function (
    callerUid: number,
    uid: number,
    uploadNames: string | string[]
  ) {
  ...
  User.collateUploads = async function (uid: number, archive: any) {
